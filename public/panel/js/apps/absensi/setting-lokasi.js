var table = NioApp.DataTable('#dt-table', {
    ...
    searchDelay: 500,
    ajax: {
        url: '/absensi/datatable-setting-lokasi',
        type: 'POST',
        data: function(d) {
            d._token = token;
        }
...
function hapus(kode) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: '/absensi/delete-setting-lokasi/'+kode,
                dataType: 'JSON',
...
$('#form-data').submit(function(e) {
    ...
    $.ajax({
        url : "/absensi/store-setting-lokasi",  
        data : formData,
        type : "POST",
        dataType : "JSON",
        cache:false,
...
function edit(kode) {
    $.ajax({
        url: '/absensi/edit-setting-lokasi/'+kode,
        dataType: 'JSON',
        success: function(response) {
            if(response.status) {
                $('#modalForm').modal('show');
...
function aktivasi(kode, flag) {
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: '/absensi/aktivasi-setting-lokasi',
                dataType: 'JSON',
                type: 'POST',
                data: {'_token': token, 'kode': kode, 'flag': flag},
                success: function(response) {
                    if(response.status){
                        $("#dt-table").DataTable().ajax.reload(null, false);
                        NioApp.Toast(response.message, 'success', {position: 'top-right'});
